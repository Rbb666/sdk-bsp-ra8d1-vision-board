...
int ml_backend_init_model(py_ml_model_obj_t *model) {
    ...
    rt_kprintf("malloc arena_memory:%#x,arena_size:%d\n", arena_memory, arena_size);
    ...
    m_free(arena_memory);
    rt_kprintf("free arena_memory,arena_size:%d\n", arena_size);
    ...
    state->interpreter = new(m_new0(MicroInterpreter, 1)) MicroInterpreter(state->model,
                                                                           *state->resolver,
                                                                           state->arena,
                                                                           arena_size);
    rt_kprintf("state->interpreter:%#08x,arena_size:%d\n", state->interpreter, arena_size);
